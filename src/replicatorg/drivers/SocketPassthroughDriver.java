...
public class SocketPassthroughDriver extends SocketDriver {
	/**
	...
	private byte[] responsebuffer = new byte[maxBufferSize*4];

	public SocketPassthroughDriver() {
		super();
	...
	public void initialize() {
		...
		if (socket == null) {
			Base.logger.severe("No Socket found.\n");
			return;
		}
		...
		if (!isInitialized()) {
			try {
				...
				Base.logger.info("Initializing Socket.");
				while (!isInitialized()) {
					readResponse();
					...
					if (now > end) {
						Base.logger.warning("Socket link non-responsive.");
						return;
					}
				}
			} catch (Exception e) {
	...
	public void readResponse() {
		...
		synchronized (socket) {
			try {
				int numread = (socket.getInputStream()).read(responsebuffer);
